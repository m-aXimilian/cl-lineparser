...
(defun duplicate-free-warnings (w f &optional d)
  "Uses the list of `warning-filter' based on warning W and input file F, removes duplicates in this, and drops everything in the output list after string D on a per line basis."
...
(defun extract-from-strings (strings &optional regex)
  ...
  (let ((results nil)
	(reg (if regex
		 regex
		 extract-from-strings-default-regexp)))
...
(defun filter-all-found-warnings/errors (f s)
  "Returns a list of lists where the first list is the statistics alist of the whole file F based on the key S, and the second list is the findings alist based on `check-all-warnings'.

Allowed values for S are `\'error' and `\'warning'. Depending on this, a list of keys will be created as the basis for the output lists. This list of keys is the result of `extract-from-strings' on the output of `duplicate-free-warnings'.
Example:
For parsing the buildoutput of a dotnet project, the desired keys are a list of stylecop/code analysis warning keys. In this case, this function reads every line and tries to find such a stylecop/code analysis key. The filter to this is a regex provided in `dotnet-stylecop-analyzer-regexp' which is the default for the `extract-from-strings' filter (This can be configured by changing `extract-from-strings-default-regexp' to different needs). For every finding of such a key, it will
- add the key to the statistics output alist and/or
- increase the counter for that key.
The second list is essentially a second loop over the output file F. This time, in each line it will be looked for the occurrence of a key from the found list of keys and if one is found.
This is not very efficient, but allows to reuse the existing API."
  (let* ((filter-key (cond
		      ((eq s 'error) ": error")
...
(defun save-compared-results (f cmp)
  "Saves the statistics output of `compare-filtered-statistics' CMP to the file F in csv-syntax."
